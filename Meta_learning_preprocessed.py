...
for file in glob.glob("data/*.*"):
    ...
    Xdf = input.drop(labels='class', axis=1)
    ydf = input["class"]
    # print(Xdf)
    X = Xdf.to_numpy()
    y = ydf.to_numpy()
    ...
    for train_index, test_index in kf.sss.split(X, y):
        ...
        # np.save(name.format(file, file), {"FrameStack": FramStack})
        np.save('processed/{}_dir/processed_{}_{}'.format(name, name, split_part), {"FrameStack": FramStack})
