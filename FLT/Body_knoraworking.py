...
def main():
    ...
    dataset_names = []
    # for files in glob.glob('processed_4/*'):
    for files in glob.glob('evaluation/datasets/*'):
        print(files)
        for items in glob.glob("{}/*.*".format(files)):
            print(items)
            name = items.split('.')[0]
            name1 = name.split('\\')[2]
            #name2 = name1.split('_')[1] + '_' + name1.split('_')[2]
            dataset_names.append(name1)
    print(dataset_names)
    ...
    for dataset_name in dataset_names:
        ensembles = [
            ("random_forest", make_random_forest(dataset_name)),
            # ("bootstrap_eole_0100_01", make_eole(100, 1,dataset_name)),
            # ("small_eole_30_Nil", make_eole(30, 1)),
            # ("bootstrap_eole_1000_01", make_eole(1000, 1)),
            # ("bootstrap_eole_0100_05", make_eole(100, 5)),
            # ("bootstrap_eole_1000_05", make_eole(1000, 5)),
        ...
        temp = dataset_name.split('.')[0]
        folder_name = temp.split("_")[0] + "_arff"
        ...
        for ens_name, ensemble in ensembles:
            ...
            experiment = Experiment(
                name=exp_name,
                ensemble=ensemble,
                dataset_loader=ArffLoader("evaluation/datasets/{}/{}.arff".format(folder_name, dataset_name)),
                n_folds=n_folds,
...
def make_random_forest(data_name):
    return EOLE(
        n_experts=100,
